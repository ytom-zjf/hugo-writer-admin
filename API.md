...
- 是否带 `Secure` 由 `config.yaml` 中的 `auth.cookieSecure` 控制：`auto`（默认，按请求是否 HTTPS 判断）、`true`、`false`
...
| `DELETE` | `/api/posts/:slug/assets/:file` | 删除文章资源文件（下次发布时生效） |
...
| 字段名 | 类型 | 说明 |
|---|---|---|
| `file` | 文件 | 必填，仅支持图片：png / jpg / jpeg / gif / webp / avif / svg，单张不超过 20MB |
...
### `DELETE /api/posts/:slug/assets/:file`

用途：删除文章 bundle 里的资源文件。
...
- 只能删除文章目录下的单个文件，`index.md` 不允许删除，非法路径返回 `400`
- 文件不存在返回 `404`
- 此接口只改动工作副本，不自动 push；删除会在下次发布时生效
- 如果正文里仍引用被删除的图片，前端会提示但不会自动改写正文
...
4. `git add --all -- content/posts/<slug>`（仅限该文章目录）
5. 若该范围内没有变更，返回 `committed: false, pushed: false`
6. 有变更时执行 `git commit -m "post: update <slug>" -- content/posts/<slug>`
7. 执行 `git push origin <branch>`

按文章发布只会提交该文章目录，其他未发布的草稿不会被一起推送。Git 命令有超时保护；错误响应会脱敏 GitHub token 和带认证的远端 URL。
...
- `configured` 表示运行所需的仓库配置是否齐全（仓库地址、发布身份、GitHub token）
- 不返回任何密码、token 等敏感信息
- `compose.yaml` 已用它配置 Docker healthcheck
